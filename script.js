...
form.addEventListener('submit',function (e){
    ...
    //After pressing the submit button make the librayr to change it;s opacity to 0.5.
...
document.querySelector(".back").addEventListener('click',()=>{
    ...
    console.log(ID);
    if(ID !== ""){
        console.log('You have neetred some book ID');
...
function animate_Selection(selected_object,camera_height,camera_orientation_y,camera_orientation_x){
    ...
    //Also when selected the mesh must have the opacity truned to 1
    if(model){
        model.traverse((child)=>{
            if(child.isMesh && child.name == 'Cube041'){
                gsap.to(
