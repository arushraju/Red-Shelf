...
function animatePopUp(){
    ...
    //Chnage the visiblity of the pop up
...
//This function rename the mesh to the appropriate one if the multiple materials are used
function rename(model){
    model.traverse((child)=>{
        if(child.name == 'Sphere002' || child.name == 'Sphere002_1'){
            child.name = 'GSAP5'; //Naming the GSAP5 appropriately.
        }
    })
}
...
function updatePercentageBar(GSAP,ID){
    ...
    //This is hard coded
    if(GSAP.name == 'GSAP5' || GSAP.name == 'GSAP4' || GSAP.name == 'GSAP3' || GSAP.name == 'GSAP1'){return 1;}
    else if(GSAP.name == 'GSAP6' || GSAP.name == 'GSAP8'){return (100/9);}
        ...
        return (200/9) + ((i+1)/(list.length + 2))*100;
    }
}
//--------------------------------------//
...
function init() {
    ...
    loader.load( 'Mesh.glb', function ( gltf ) {
        ...
        //Since there are different names of the material due toi different material I will rename it using this function
        rename(model);
...
function changeOutline(visiblity){
    let mesh = model.getObjectByName('Cube041');
...
function animate() {
    ...
    //console.log(`next gsap = ${next_GSAP}`)
