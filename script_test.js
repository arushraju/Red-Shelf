...
function isPassword(pass){
    //here I will be using the authenciation for checking the password
    if(pass == "Arush"){
        return true;
    }
}
...
admin_form.addEventListener('submit', async function (e) {
    ...
    //Check if the pass word is the right one (From the environment variables)
    if(isPassword(pass)){
        ...
        document.querySelector('.login-fail-position').style.display = 'block';
    }
...
form.addEventListener('submit',async function (e){
    ...
    if(isNaN(ID)){
        ...
        ID = Number(ID);
        //This will be a single bookshelf
        bookshelf = await getTargetBookshelf(ID);
    }
    ...
    let bookshelf_number = bookshelf.slice(0,-3);
 ...
 * @param {number} bookshelf_index - This is the index of the bookshelf. This will be an array.
...
function init() {
    ...
    scene.add(transformControls.getHelper()); //line 3698
...
document.querySelector('.crowd-option').addEventListener('click',()=>{
    ...
    document.querySelector('.crowd-option-drop-box').style.display = 'flex';
...
//-----------------[3 to 0]----------------//
document.querySelector('.crowd-floor-back-button').addEventListener('click',()=>{
    //Update the application number
    if(application_flow != 0 ){
        previuos_application_flow = 3;
        application_flow = 0;
    }
    ...
    document.querySelector('.crowd-option-drop-box').style.display = 'none';
    document.querySelectorAll('.main-menu-option').forEach((child)=>{
        child.style.display = 'flex';
    })
});
...
window.addEventListener('mousedown', (event) => {
    //Only if we are in the admin mode
    if(application_flow == 4){
        if(transform_clickedObject) transform_clickedObject.material.emissive.set(0xff4d4d);
        // Attach the gizmo to the clicked object
        //transformControls.attach(transform_clickedObject);
        //console.log("Transform attached to:", transformControls.object);
