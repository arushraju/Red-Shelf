...
function makeGrid(){
    const grid = { 
        ...
        //Second Floor
        "GSAP_3_1" : makeGridObejct('GSAP_3_1',['GSAP_2_7','GSAP_3_2','GSAP_3_8','GSAP_3_9','GSAP_3_13','GSAP_3_12','GSAP_3_11','GSAP_3_10','GSAP_3_14']),
        ...
        "GSAP_3_11" : makeGridObejct('GSAP_3_11',['GSAP_3_1']),
...
const first_floor_fire_extinguisher = ['Fire_Extinguisher_11_Frustum','Fire_Extinguisher_10_Frustum','Fire_Extinguisher_12_Frustum','Fire_Extinguisher_13_Frustum','Fire_Extinguisher_14_Frustum'];
...
const first_floor_computer = ['Computer_11_Frustum','Computer_12_Frustum'];
...
function makeVisible(floor){
    ...
    if(floor == 'Basement_Floor'){
        ...
        for(let i=0; i < first_floor_tables_2_instanceMesh.total;i++){setInstanceScale(i,0,first_floor_tables_2_instanceMesh);}
                ...
                gsap.to(
                    child.scale,
                    {
                        ...
                        onComplete : ()=>{
                            ...
                            // Firts Floor 2 Table scale reset
                            for(let i=0; i < first_floor_tables_2_instanceMesh.total;i++){setInstanceScale(i,1,first_floor_tables_2_instanceMesh);}
...
//-------------[4.1 to 4.3]--------------//
document.querySelector('.bookshelf-admin-option').addEventListener('click',()=>{
    console.log('You are inside the booklshef option')
    ...
    //Disable all the Frustum and GSAP and enable all the bookshelfs
    //draggableObjects.push(first_floor_bookshelf_object);
...
document.querySelector('.admin-save-button').addEventListener('click',async ()=>{
    ...
    loading_page.querySelector('.loading-text').textContent = 'Saving Database...';
    ...
    for(let i = 0; i<first_floor_bookshelf_name.length; i++){
        ...
        bookshelf = model.getObjectByName(bookshelf_name);
...
document.querySelector('.admin-cancel-button').addEventListener('click',()=>{
    //When cancel button is pressed, then just log out and do not change the databse at all. But before that change the bookshelfs to the default ones, and then log out
})

/**
 * This function will help to update the UI such that the admin can change the range of the bookshelf.
 * @param {object} clicked_obejct - this is the obejct that is clicked and has the transform helper located
 */
async function updateTransformClicked(clicked_obejct){
    if(first_floor_bookshelf_name.includes(clicked_obejct.name)){
        let {data , error} = await supabase
            .from('bookshelf_database')
            .select('range_high, range_low')
            .eq('mesh_name',(String)(clicked_obejct.name))
...
form.addEventListener('submit',async function (e){
    ...
    if(isNaN(ID)){
        ...
        ID = String(ID);
        //Check which subject does it match
        getSubjectBookshelf(ID);
    ...
    .then(data =>{
        ...
        if(first_time){
            for(let i=0;i<jsonData.length;i++){
                subjects.push(jsonData.Subject);
                subjects_ID.push(jsonData.Subject_ID);
            }
            first_time = false;
        }

        //Find the searched text in the array
        let index = 0;
        for(;index<subjects.length;index++){
            if(String(subjects[index]).toLowerCase() === String(subject_text).toLowerCase()){
        ...
        let bookshelf_subject_ID = subjects_ID[index];

        console.log(bookshelf_subject_ID);
    })
}
...
// Names of all the obejcts that will be used to get the 
...
const ground_library_Frustum = [
    ...
    'Baggage_Rack',
    'Security_Frustum',
    'Computer_Reading_Room_Frustum',
    'Restricted_Room_Frustum',
    'Drop_Box_1_Frustum',
    'Drop_Box_2_Frustum',
    'Computer_Desk_Frustum',
    'Female_Washroom_0_Frustum',
    'Male_Washroom_0_Frustum',
    'Water_0_Frustum',
    'Reception_Frustum',
    'Seminar_Room_Frustum',
    'Stationary_Shop_Frustum',
    'Text_Books_Frustum',
    'Lift_Frustum',
    "Dustbin",
    'AHU_Room_Frustum',
    'Basement_Reading_Room_Frustum',
];
...
const second_library_Frustum = [
    'Room301_Frustum',
    'Srihari_KeshavMurthy_Frustum',
    'Committe_Frustum',
    'Library_Office_Frustum',
    'Bound_Periodical_Frustum',
    'Room307_Frustum',
    'Room308_Frustum',
];

const allFrustum = [
    ...outer_library_Frustum,
    ...ground_library_Frustum,
    ...first_library_Frustum,
    ...second_library_Frustum,
]

//These are the pop up divs of the outer library
const dustbin_1_pop_up = document.querySelector('#Dustbin_1_Frustum');
const dustbin_2_pop_up = document.querySelector('#Dustbin_2_Frustum');
const dustbin_3_pop_up = document.querySelector('#Dustbin_3_Frustum');
const dustbin_4_pop_up = document.querySelector('#Dustbin_4_Frustum');
const library_entrance = document.querySelector('#Entrance_Library_Frustum');
const entrance_1_pop_up = document.querySelector('#Entrance_1_Frustum');
const entrance_2_pop_up = document.querySelector('#Entrance_2_Frustum');
const entrance_3_pop_up = document.querySelector('#Entrance_3_Frustum');
const entrance_4_pop_up = document.querySelector('#Entrance_4_Frustum');
const entrance_5_pop_up = document.querySelector('#Entrance_5_Frustum');
const entrance_6_pop_up = document.querySelector('#Entrance_6_Frustum');


//These are the divs of the ground floor of the library
const kiosk_pop_up = document.querySelector('#Kiosk_Frustum');
const verification_desk_pop_up = document.querySelector('#Verification');
const baggage_rack_pop_up = document.querySelector('#Baggage_Rack');
const reading_room_computer_pop_up = document.querySelector('#Reading_Room_Computers_Frustum');
const unknown_room_1_pop_up = document.querySelector('#Restricted_Room_Frustum');
const drop_box_1_pop_up = document.querySelector('#Drop_Box_1_Frustum');
const drop_box_2_pop_up = document.querySelector('#Drop_Box_2_Frustum');
const computer_desk_frustum = document.querySelector('#Computer_Desk_Frustum');
const female_washroom_pop_up = document.querySelector('#Female_Washroom_Frustum');
const male_washroom_pop_up = document.querySelector('#Male_Washroom_0_Frustum');
const water_0_pop_up = document.querySelector('#Water_0_Frustum');
const reception_pop_up = document.querySelector('#Reception_Frustum');
const seminar_pop_up = document.querySelector('#Seminar_Room_Frustum');
const stationary_pop_up = document.querySelector('#Stationary_Shop_Frustum');
const text_books_pop_up = document.querySelector('#Text_Books_Frustum');
const lift_0_pop_up = document.querySelector('#Lift_Frustum');
const dustbin_0_pop_up = document.querySelector('#Dustbin_0_Frustum');
const ahu_room_pop_up = document.querySelector('#AHU_Room_Frustum');
const basement_way = document.querySelector('#Basement_Reading_Room_Frustum');

//These are the divs of the first floor of the library
const lectern_1_pop_up = document.querySelector('#Lectern_1_Frustum');
const lectern_2_pop_up = document.querySelector('#Lectern_2_Frustum');
const lectern_3_pop_up = document.querySelector('#Lectern_3_Frustum');
const lectern_4_pop_up = document.querySelector('#Lectern_4_Frustum');
const search_computer_1_pop_up = document.querySelector('#Computer_11_Frustum');
const search_computer_2_pop_up = document.querySelector('#Computer_12_Frustum');
const office_1_pop_up = document.querySelector('#S_K_Vijaianand_Office_Frustum');
const room_unknown_2_pop_up = document.querySelector('#Room_Frustum');
const water_1_pop_up = document.querySelector('#Water_1_Frustum');
const male_washroom_1_frustum = document.querySelector('#Male_Washroom_1_Frustum');
const female_washroom_1_frustum = document.querySelector('#Female_Washroom_1_Frustum');
const lift_1_pop_up = document.querySelector('#Lift_1_Frustum');
const server_pop_up = document.querySelector('#Server_Room_Frustum');

//These are the dives of the second floor of the library
const room_301_pop_up = document.querySelector('#Room301_Frustum');
const office_2_pop_up = document.querySelector('#Srihari_KeshavMurthy_Frustum');
const committe_room_pop_up = document.querySelector('#Committe_Frustum');
const library_office_pop_up = document.querySelector('#Library_Office_Frustum');
const bound_periodical_pop_up = document.querySelector('#Bound_Periodical_Frustum');
const room_307_pop_up = document.querySelector('#Room307_Frustum');
const room_308_pop_up = document.querySelector('#Room308_Frustum');

const frustum_info =  {

    //Floor -1
    "Entrance_4" : {
        div : entrance_4_pop_up,
        quick_search : "Entry_Way",
        floor : -1,
        gsap_name : "GSAP_0_3",
        isDisplay : false,
    },
    "Entrance_1" : {
        div : entrance_1_pop_up,
        quick_search : "Entry_Way",
        floor : -1,
        gsap_name : "GSAP_0_14",
        isDisplay : false,
    ...
    "Dustbin_1" : {
        div : dustbin_1_pop_up,
        quick_search : "Dustbin",
        floor : -1,
        gsap_name : "GSAP_0_14",
        isDisplay : false,
    },
    "Dustbin_2" : {
        div : dustbin_2_pop_up,
        quick_search : "Dustbin",
        floor : -1,
        gsap_name : "GSAP_0_10",
        isDisplay : false,
    ...
    "Kiosk_Frustum" : {
        div : kiosk_pop_up,
        quick_search : "Machine",
        floor : 0,
        gsap_name : "GSAP_1_14",
        isDisplay : false,
    },
    "Baggage_Rack" : {
        div : baggage_rack_pop_up,
        quick_search : "",
        floor : 0,
        gsap_name : "",
        isDisplay : false,
    ...
    "Computer_Reading_Room_Frustum" : {
        div : reading_room_computer_pop_up,
        quick_search : "Computer",
        floor : 0,
        gsap_name : "GSAP_1_30",
        isDisplay : false,
    },
    "Restricted_Room_Frustum" : {
        div : unknown_room_1_pop_up,
        quick_search : "",
        floor : 0,
        gsap_name : "GSAP_1_29",
        isDisplay : false,
    ...
    'Computer_Desk_Frustum' : {
        div : computer_desk_frustum,
        quick_search : "Help_Desk",
        floor : 0,
        gsap_name : "GSAP_1_7",
        isDisplay : false,
    },
    "Female_Washroom_0_Frustum" : {
        div : female_washroom_pop_up,
        quick_search : "Utility",
        floor : 0,
        gsap_name : "GSAP_1_22",
        isDisplay : false,
    },
    'Male_Washroom_0_Frustum' : {
        div : male_washroom_pop_up,
        quick_search : "Utility",
        floor : 0,
        gsap_name : "GSAP_1_21",
        isDisplay : false,
    ...
    'Text_Books_Frustum' : {
        div : text_books_pop_up,
        quick_search : "Releases",
        floor : 0,
        gsap_name : "GSAP_1_14",
        isDisplay : false,
    },
    ...
    //Floor 1
    "Lectern_1_Frustum" : {
        div : lectern_1_pop_up,
        quick_search : "Releases",
        floor : 1,
        gsap_name : ['GSAP_2_3','GSAP_2_2'],
        isDisplay : false,
    },
    "Lectern_2_Frustum" : {
        div : lectern_2_pop_up,
        quick_search : "Releases",
        floor : 1,
        gsap_name : ['GSAP_2_4','GSAP_2_3'],
        isDisplay : false,
    },
    "Lectern_3_Frustum" : {
        div : lectern_3_pop_up,
        quick_search : "Releases",
        floor : 1,
        gsap_name : ['GSAP_2_3','GSAP_2_4'],
        isDisplay : false,
    },
    "Lectern_4_Frustum" : {
        div : lectern_4_pop_up,
        quick_search : "Releases",
        floor : 1,
        gsap_name : ['GSAP_2_4','GSAP_2_5'],
        isDisplay : false,
    },
    'Computer_11_Frustum' : {
        div : search_computer_1_pop_up,
        quick_search : "Computer",
        floor : 1,
        gsap_name : 'GSAP_2_12',
        isDisplay : false,
    ...
    'Male_Washroom_1_Frustum' : {
        div : male_washroom_1_frustum,
        quick_search : "Utility",
        floor : 1,
        gsap_name : "GSAP_2_10",
        isDisplay : false,
    },
    'Female_Washroom_1_Frustum' : {
        div : female_washroom_1_frustum,
        quick_search  :"Utility",
        floor : 1,
        gsap_name : "GSAP_2_17",
        isDisplay : false,
...
//This functino will find he distance between the camera dn some object
        ...
        if (frustum.containsPoint(position)) {
            ...
            if(application_flow == 1 || application_flow == 2.3 || application_flow == 2.4){
                ...
                if(
                    ...
                    distance_Cam_and_obj(camera,obj) < 300
                ){
...
function init() {
    ...
    // Loading the World
    loader.load( './Mesh_Test.glb', async function( gltf ) {
        model = gltf.scene;
        ...
        First_Floor_Table_2_Instance = gltf.scene.getObjectByName('First_Floor_Table_2_instance');
        ...
        createInstances(First_Floor_Table_2_Instance.geometry,First_Floor_Table_2_Instance.material,loadFirstFloorTables2Transform)//First Floor 2 Tables
...
document.querySelector('.crowd-miscellaneous-option').addEventListener('click',()=>{
    ...
    setColor(first_floor_tables_2_instanceMesh,first_floor_occupied_seats);
...
document.querySelector('.miscellaneous-floor-back-button').addEventListener('click',()=>{
    ...
    if(previuos_application_flow == 3.1){
        ...
        ColorReset(first_floor_tables_2_instanceMesh);
        ColorReset(first_floor_tables_2_instanceMesh);
...
let first_floor_tables_2_instanceMesh = {};
...
async function loadFirstFloorTablesTransform() { 
    const response = await fetch('./Instances/First_Floor_Table_Instance.json');
    return await response.json();
}
...
async function loadFirstFloorTables2Transform(){
    const response = await fetch('./Instances/First_Floor_Table_2_Instance.json');
    return await response.json();
}
...
async function createInstances(geometry, material,loadTransforms) {
    ...
    } else if(geometry == First_Floor_Table_2_Instance.geometry) {
        first_floor_tables_2_instanceMesh = {
            instance : instancedMesh,
            transform : [],
            total : count
        }
    ...
    for (let i = 0; i < count; i++) {
        ...
        if(geometry == First_Floor_Table_Instance.geometry){
            ...
            first_floor_tables_2_instanceMesh.transform.push(
                {
                    position : dummy.position.clone(),
                    rotation : dummy.rotation.clone(),
                    scale : dummy.scale.clone(),
...
async function update_Bookshelf(model) {
    ...
    for(let i=0; i < first_floor_bookshelf_name.length;i++){
        ...
        //First get the values of the position and the scale from the 

        //Call the databse api
        let {data,error} = await supabase
            .from('bookshelf_database')
            .select('x_pos, y_pos, z_pos, x_rot, y_rot, z_rot, x_sca, y_sca, z_sca')
            .eq('mesh_name',bookshelf.name);
        
        //Update the posiiton of the mesh based on the database
        bookshelf.position.set(data[0].x_pos,data[0].y_pos,data[0].z_pos);
        bookshelf.rotation.set(data[0].x_rot,data[0].y_rot,data[0].z_rot);
        bookshelf.scale.set(data[0].x_sca,data[0].y_sca,data[0].z_sca);

        //console.log(`bookshelf name is ${bookshelf.name}`);
...
function animate() {
    ...
    //console.log(`Orbital Controls : ${orbitalControls.enabled} and Transform Controls : ${transformControls.enabled}`);
