...
basement_floor_library_child.push('Cube360');
...
/**
 * This functino will connect to the API and will then check the credintial to see if the user is valid or not (Admin)
 * @param {id} - This is the email id that the user has entered and that will be checked
...
async function testAPI(id,pass) {
    ...
    if(error){
        //Return the message of incorrect password
        console.error(error);
        return false;
    } else{
        console.log(data);
...
document.querySelector('.cross').addEventListener('click',()=>{
    ...
    //Empty the Helper pop up (To avoid the helper content and the model canvas height problem)
...
//document.querySelector('.help').addEventListener('click', onHelpClick )
document.querySelector('.help-image').addEventListener('click', onHelpClick );
...
document.querySelector('.start-location-search-back-button').addEventListener('click',()=>{
    ...
    //Also if the bookshelf was selected and the user never choose the starting location
    ID = null;
    bookshelf_ID = null;
    bookshelf_side = null;
    bookshelf = null;
...
//---- FRUSTUM-----//
function updateBookCanvas(){
    const book_model = document.querySelector('.bookshelf-model');
    let canvas = document.querySelector('.book-search-canvas');
...
async function createFrustumInfo(){
    ...
    if(!error){
        ...
        for(let i = 0; i < data.length; i++){
            ...
            frustum_div.addEventListener('click',async (e)=>{
                ...
                if(first_floor_bookshelf_name.includes(e.currentTarget.id)){
                    ...
                    if((Number)(bookshelf_ID) && (Number)(bookshelf_ID) >= 1 && (Number)(bookshelf_ID) <= 54){
                        ...
                        await shownBookshelf(bookshelf_side , (Number)(bookshelf_ID));
                        updateBookCanvas();
                        ...
                        console.log(`You are in the bookshelf pop up, and their is no bookshelf side | bookshelf number = ${bookshelf_ID} | side = ${bookshelf_side}`)
                        await shownBookshelf(null , (Number)(e.currentTarget.id));
                        updateBookCanvas();
                    }
                }
        ...
        allTables_object.forEach((table)=>{
            if(table.name == data[i].table_mesh_name){
                if(data[i].isoccupied){
                    table.material.color.set(0x66FF00);// Green
                } else {
                    table.material.color.set(0xFF0000);// Red
                }
            }
        })
    }
...
document.querySelector('.crowd-miscellaneous-option').addEventListener('click',async ()=>{
    ...
    //Make the helper pop up for this crwod to eb visible
    onHelpClick(application_flow);
});
...
document.querySelector('.charging-miscellaneous-option').addEventListener('click',()=>{
    ...
    //Show the helper option immediately after entering this
    onHelpClick();
});

//-------------[3 to 3.3 to 3]--------------//
document.querySelector('.switch-manually-option').addEventListener('click',()=>{
    //Update the application flow 
    if(application_flow != 3.3){
        previuos_application_flow = application_flow;
        application_flow = 3.3;
    }

    //just show what I want to do.
    onHelpClick();

    //Then again make the appluication flow to 3
    application_flow = 3;
})
            ...
            { 
                ...
                onComplete: ()=>{
                    ...
                    helper_pop_up_init(controls).then(async ()=>{
                        ...
                        console.log('The Bookshelf Mesh is loaded');
                    });
