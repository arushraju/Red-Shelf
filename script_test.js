...
//------------- A STAR ALGORITHM -------------//
let starting_location;//This will store the name of GSPA at the strat
...
const outer_nodes = [];
const ground_floor_nodes = [];
const first_floor_nodes = [];
const second_floor_nodes = [];
...
for(let floor = -1 ; floor <= 2 ; floor++){
    ...
    if(floor == -1){
        for(let i = 1 ;i <= 14; i++){
            ...
            outer_nodes.push(`Node_${floor+1}_${i}`)
        }
    }
    //Filling the GSAP of Ground Floor
        ...
        for(let i = 4 ;i <= 33; i++){
            ...
            ground_floor_nodes.push(`Node_${floor+1}_${i}`)
        }
    }
    //Filling the GSAP of First flor
        ...
        for(let i = 1 ;i <= 16; i++){
            ...
            first_floor_nodes.push(`Node_${floor+1}_${i}`);
        }
    } else if(floor == 2){
        for(let i = 1 ;i <= 7; i++){
            second_floor_GSAP.push(`GSAP_${floor+1}_${i}`);
            second_floor_nodes.push(`Node_${floor+1}_${i}`);
        }
    }
}
...
const allNodes = [...outer_nodes,...ground_floor_nodes,...first_floor_nodes,...second_floor_nodes];
...
/**
 * This function will enlarge or shrink the Nodes and Edges of a particular Floor
 * @param {*} model - gltf.scene
 * @param {number} floor - FLoor Number (-1,0,1,2) 
 * @param {boolean} targetted - true if the nodes and edges are in path, false if all gsap is targetted.
...
function controlEdgesNodes(model,floor,targetted,makeVisible){
    ...
    switch (floor) {
        case -1 :
            ...
            floor_node = outer_nodes;
            break;
        case 0 :
            floor_edge = ground_floor_edges;
            floor_node = ground_floor_nodes;
            break;
        case 1 :
            floor_edge = first_floor_edges;
            floor_node = first_floor_nodes;
            break
        case 2 :
            floor_edge = second_floor_edges;
            floor_node = second_floor_nodes;
            break;
        default:
    ...
    //If the edges and nodes at the path has to be controlled
    if(targetted){
    } else {
        //Edges and Ndoe of the outer library
        model.traverse((child)=>{
            if(floor_edge.includes(child.name) || floor_node.includes(child.name)){
                gsap.to(
                    child.scale,
                    {
                        x:  scale,
...
ground_floor.forEach((child)=>{
    child.addEventListener('click',()=>{
        ...
        if(application_flow == 1){
            ...
            //GSPA will surely be disabled
            //Make all the edges and nodes to be disabled
            controlEdgesNodes(model,-2,false,false);
            //Make all the Nodes and edges of that floor to be triggered
            controlEdgesNodes(model,current_floor,false,true);
        }
    });
})
...
function onGSAPselectionExplore(gsap){
    ...
    //Activate the Frusutm depending on the gsap and also make all the GSAP to appear.
...
function onGsapSelectionQuickSearch(gsap){
    ...
    //Update the title
    document.querySelector('.title-content').innerHTML = 'Choose the Starting Location [2.4]';
}
...
function onGsapSelectionLocationShown(gsap){
    ...
    //Make only the ending point GSAP to turn on.
    ...
    //Update the title
    const title_content = 'You are in the Location Shwon (GSAP Mode)';
    document.querySelector('.title-content').textContent = title_content;
}
...
function focusFloor(camera,controls,floor){
    const pos = new THREE.Vector3();
    if(floor === 'Ground_Floor'){
        console.log('Focus of the Ground Floor');
        

        //get the posiiton of the ground floor origin.
        const ground_floor_origin = model.getObjectByName('') 

    } else if (floor === 'First_Floor') {
        console.log('Focus of the First Floor');
    } else if (floor === 'Secomd_Floor') {
...
function init() {
    ...
    // Loading the World
    loader.load( './Test_Mesh.glb', function ( gltf ) {
        model = gltf.scene;
